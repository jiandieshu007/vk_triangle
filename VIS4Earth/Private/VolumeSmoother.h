...
class VIS4EARTH_API FVolumeSmoothShader : public FGlobalShader {
  public:
    ...
    SHADER_PARAMETER(FIntVector, VolDim)
...
class VIS4EARTH_API FVolumeSmoother {
  ...
  private:
    static void exec(FRHICommandListImmediate &RHICmdList, const Parameters &Params) {
        ...
        FString SmoothedVolumeBufferString = FString(TEXT("Smoothed Volume Buffer")) +
                                             TEXT(" in ") + FString(ANSI_TO_TCHAR(__FUNCTION__));
        FString VolumeTextureString =
            FString(TEXT("Volume Texture")) + TEXT(" in ") + FString(ANSI_TO_TCHAR(__FUNCTION__));
        FString ReadbackSmoothedVolumeString = FString(TEXT("Readback Smoothed Volume")) +
                                               TEXT(" in ") + FString(ANSI_TO_TCHAR(__FUNCTION__));
        const TCHAR *SmoothedVolumeBufferTChar = *SmoothedVolumeBufferString;
        const TCHAR *VolumeTextureTChar = *VolumeTextureString;
        const TCHAR *ReadbackSmoothedVolumeTChar = *ReadbackSmoothedVolumeString;

        FRDGBuilder grphBldr(RHICmdList);

        auto shaderParams = grphBldr.AllocParameters<FVolumeSmoothShader::FParameters>();
        FIntVector volDim(Params.VolumeTexture->GetSizeX(), Params.VolumeTexture->GetSizeY(),
            ...
            SmoothedVolumeBufferTChar);
        {
            ...
            auto extrnlTexRDG = RegisterExternalTexture(
                ...
                VolumeTextureTChar);
            FRDGTextureSRVDesc TMD;
            TMD.Create(extrnlTexRDG);
            shaderParams->VolInput = grphBldr.CreateSRV(TMD);
        ...
        auto bufReadback = new FRHIGPUBufferReadback(ReadbackSmoothedVolumeTChar);
        AddEnqueueCopyPass(grphBldr, bufReadback, smoothedVolBuf,
                           smoothedVolBuf->Desc.GetTotalNumBytes());
